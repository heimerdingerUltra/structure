...
from src.models.registry import ModelMetadata, ModelRegistry, create_version_string
...
def parse_args():
    ...
    parser.add_argument(
        "--strategy", type=str, default="balanced", choices=["fast", "accurate", "balanced"]
    )
    ...
    parser.add_argument(
        "--env", type=str, default="production", choices=["development", "staging", "production"]
    )
...
def train_single_model(
    ...
    trainer.fit(train_loader, val_loader, config.epochs, verbose=10)
...
def main():
    ...
    train_loader, val_loader, test_loader = pipeline.create_loaders(
        X, y, test_size=0.2, val_size=0.1, seed=args.seed
    )
    ...
    for model_name in args.models:
        model, metrics = train_single_model(
            model_name, n_features, train_loader, val_loader, test_loader, config, env, device
        )
    ...
    if len(trained_models) > 1:
        ...
        ensemble = AdvancedEnsembleSystem(
            trained_models, device=device, ensemble_type=args.ensemble_type
        )
        ...
        ensemble.train_combiner(
            train_loader, val_loader, optimizer, criterion, n_epochs=100, patience=15, verbose=True
        )
    ...
    for name, model in trained_models.items():
        ...
        registry.register_model(model, metadata, scaler=fe.scaler, feature_names=fe.feature_names)

    summary = MetricsSummary.create_summary(
        test_y, ensemble_pred if len(trained_models) > 1 else pred
    )
