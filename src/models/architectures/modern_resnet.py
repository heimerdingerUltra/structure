...
from .normalization import LayerScale, RMSNorm
...
class BottleneckBlock(nn.Module):
    ...
    def __init__(self, dim: int, expansion: int = 4, dropout: float = 0.0, use_se: bool = True):
        super().__init__()
...
class ModernResNet(nn.Module):
    ...
    def __init__(
        ...
        self.stem = nn.Sequential(nn.Linear(n_features, dim), RMSNorm(dim), nn.SiLU())
        ...
        self.blocks = nn.ModuleList(
            [
                ResidualBlock(
                    dim, int(dim * hidden_dim_ratio), dropout, dpr[i], use_se, use_layer_scale
                )
        ...
        self.head = nn.Sequential(
            nn.Linear(dim, dim // 2), nn.SiLU(), nn.Dropout(dropout), nn.Linear(dim // 2, 1)
        )
...
class PyramidResNet(nn.Module):
    ...
    def __init__(
        ...
        self.stem = nn.Sequential(nn.Linear(n_features, dims[0]), RMSNorm(dims[0]), nn.SiLU())
