...
class mLSTMCell(nn.Module):
    ...
    def forward(self, x: torch.Tensor, state: tuple) -> tuple:
        ...
        h_tilde_raw = torch.bmm(C, q_expanded).squeeze(2)

        norm_denom = torch.sum(n * q, dim=1, keepdim=True).clamp(min=1.0)
        h_tilde = h_tilde_raw / norm_denom
...
class xLSTMBlock(nn.Module):
    def __init__(
        self, input_size: int, hidden_size: int, use_mlstm: bool = True, dropout: float = 0.1
    ):
        super().__init__()
...
class xLSTM(nn.Module):
    def __init__(
        ...
        self.blocks = nn.ModuleList(
            [xLSTMBlock(hidden_size, hidden_size, use_mlstm, dropout) for _ in range(n_layers)]
        )
