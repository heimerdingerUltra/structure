...
from .attention import LinearAttention, MultiQueryAttention
...
class ModernTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList(
            [
                TransformerBlock(
                    d_model, n_heads, mlp_ratio, dropout, attention_type, use_layer_scale
                )
...
class ParallelTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.mlp_blocks = nn.ModuleList(
            [SwiGLU(d_model, int(d_model * mlp_ratio)) for _ in range(n_layers)]
        )
