    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.prenormalization:
            ...
            x = self.attention_norm(
                x + self.attention_residual_dropout(self.attention(x, x, x, need_weights=False)[0])
            )
...
class SAINT(nn.Module):
    ...
    def __init__(
        ...
        self.inter_sample_blocks = nn.ModuleList(
            [FTTransformerBlock(d_token, n_heads, d_ffn, dropout, dropout) for _ in range(n_blocks)]
        )
...
class TabNet(nn.Module):
    ...
    def __init__(
        ...
        self.initial_splitter = nn.Sequential(
            nn.Linear(n_features, n_d + n_a), nn.BatchNorm1d(n_d + n_a, momentum=momentum)
        )
