...
class AttentionEnsemble(nn.Module):
    ...
    def forward(
        self, predictions: torch.Tensor, target: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
...
class HierarchicalEnsemble(nn.Module):
    ...
    def __init__(self, n_models: int, n_groups: int = 2):
        ...
        self.meta_combiner = nn.Sequential(
            nn.Linear(n_groups, 16), nn.LayerNorm(16), nn.GELU(), nn.Dropout(0.1), nn.Linear(16, 1)
        )
...
class AdvancedEnsembleSystem:
    ...
    def __init__(
        self, models: Dict[str, nn.Module], device: str = "cuda", ensemble_type: str = "attention"
    ):
