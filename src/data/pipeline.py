...
class DataPipeline:
    ...
    def create_loaders(
        ...
        train_loader = self._create_loader(Subset(dataset, train_idx), shuffle=True)

        val_loader = self._create_loader(Subset(dataset, val_idx), shuffle=False)

        test_loader = self._create_loader(Subset(dataset, test_idx), shuffle=False)

        return train_loader, val_loader, test_loader

    def create_kfold_loaders(self, X: np.ndarray, y: np.ndarray, n_splits: int = 5, seed: int = 42):
