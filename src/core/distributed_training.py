...
class DistributedOrchestrator:
    ...
    def __init__(
        ...
        if self.world_size > 1:
            dist.init_process_group(
                backend=backend, init_method=init_method, world_size=self.world_size, rank=self.rank
            )
    ...
    def wrap_model(self, model: nn.Module, device_ids: Optional[list] = None) -> nn.Module:
        ...
        model = DDP(
            model, device_ids=device_ids, output_device=device_ids[0], find_unused_parameters=False
        )
