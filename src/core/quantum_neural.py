...
class QuantumInspiredLinear(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        superposed_weight = torch.sum(modulated_weights * coeffs.view(self.n_basis, 1, 1), dim=0)
...
class AdaptiveComputationTime(nn.Module):
    ...
    def forward(
        self, core_fn, x: torch.Tensor, max_steps: int = 10
        ...
        for step in range(max_steps):
            ...
            new_halted = (
                halting_prob + p * still_running >= self.threshold
            ).float() * still_running
...
class NeuralODE(nn.Module):
    ...
    def __init__(self, dim: int, n_steps: int = 6):
        ...
        self.dynamics = nn.Sequential(nn.Linear(dim, dim * 2), nn.Tanh(), nn.Linear(dim * 2, dim))
...
class SelfModulatingNeuron(nn.Module):
    ...
    def __init__(self, dim: int):
        ...
        self.meta_net = nn.Sequential(nn.Linear(dim, dim // 4), nn.GELU(), nn.Linear(dim // 4, 3))
    ...
    def __init__(self, dim: int, depth: int = 3, share_weights: bool = True):
        ...
        if share_weights:
            self.recurrent_block = nn.Sequential(
                nn.Linear(dim, dim * 2), nn.LayerNorm(dim * 2), nn.GELU(), nn.Linear(dim * 2, dim)
            )
        else:
...
class NeuralTuringMachine(nn.Module):
    ...
    def __init__(
        self, input_dim: int, hidden_dim: int, memory_size: int = 128, memory_dim: int = 64
    ):
        super().__init__()
        ...
        self.read_head = nn.Sequential(nn.Linear(hidden_dim, memory_dim + 1 + 1 + 3), nn.Sigmoid())

        self.write_head = nn.Sequential(
            nn.Linear(hidden_dim, memory_dim + 1 + 1 + 3 + memory_dim + memory_dim), nn.Sigmoid()
        )
