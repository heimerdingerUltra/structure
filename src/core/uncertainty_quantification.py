...
class BayesianLinear(nn.Module):
    ...
    def _kl_normal(
        self, mu1: torch.Tensor, sigma1: torch.Tensor, mu2: float, sigma2: float
    ) -> torch.Tensor:
...
class DeepEnsemble(nn.Module):
    ...
    def __init__(self, base_model: nn.Module, n_models: int = 5):
        super().__init__()
        self.models = nn.ModuleList([self._copy_model(base_model) for _ in range(n_models)])
            ...
            if self.n_averaged == 0:
                ...
                for group in self.base_optimizer.param_groups:
                    for p in group["params"]:
                        ...
                        self.mean[p] = (self.n_averaged * self.mean[p] + p.data) / (
                            self.n_averaged + 1
                        )
                        self.sq_mean[p] = (self.n_averaged * self.sq_mean[p] + p.data**2) / (
                            self.n_averaged + 1
                        )
...
class EvidentialRegression(nn.Module):
    ...
    def __init__(self, base_model: nn.Module):
        ...
        self.evidence_head = nn.Sequential(
            nn.Linear(last_dim, last_dim // 2), nn.ReLU(), nn.Linear(last_dim // 2, 4)
        )
