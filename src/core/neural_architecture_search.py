...
class DilConv(nn.Module):
    def __init__(
        self, in_c: int, out_c: int, kernel: int, stride: int, padding: int, dilation: int
    ):
        super().__init__()
        self.op = nn.Sequential(
    ...
    def __init__(self, in_features: int, hidden_dims: List[int], n_ops: int = 5):
        ...
        for hidden_dim in hidden_dims:
            layer_ops = nn.ModuleList(
                [
                    nn.Sequential(
                        nn.Linear(prev_dim, hidden_dim), nn.BatchNorm1d(hidden_dim), nn.ReLU()
                    )
        ...
        self.arch_params = nn.ParameterList(
            [nn.Parameter(torch.randn(n_ops)) for _ in range(len(hidden_dims))]
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(1, 4):
            ...
            for j in range(i):
                ...
                node_input.append(sum(w * op(nodes[j]) for w, op in zip(weights, edge_ops)))
...
class SuperNet(nn.Module):
    ...
    def __init__(
        self, in_features: int, out_features: int, max_depth: int = 6, width_multiplier: float = 1.0
    ):
        super().__init__()
