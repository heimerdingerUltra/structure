            ...
            for param, w_before in zip(self.model.parameters(), weights_before):
                avg_adapted = torch.stack(
                    [
                        ...
                        for i, _ in enumerate(weights_before)
                        if id(param) == id(w_before)
                    ]
                ).mean(0)
...
class ProtoNet(nn.Module):
    ...
    def compute_prototypes(self, embeddings: Tensor, labels: Tensor) -> Tensor:
        ...
        prototypes = torch.stack(
            [embeddings[labels == label].mean(dim=0) for label in unique_labels]
        )
        return prototypes
...
class MatchingNet(nn.Module):
    ...
    def forward(self, support_x: Tensor, support_y: Tensor, query_x: Tensor) -> Tensor:
        ...
        similarities = torch.mm(attended_query.squeeze(1), support_embeddings.t())

        attention = F.softmax(similarities, dim=-1)

        predictions = torch.mm(
            attention, support_y.unsqueeze(-1) if support_y.dim() == 1 else support_y
        )
...
class TaskEmbedding(nn.Module):
    ...
    def forward(self, support_x: Tensor, support_y: Tensor) -> Tensor:
        support_stats = torch.cat([support_x.mean(dim=0), support_x.std(dim=0)])
...
class ConditionalNeuralProcess(nn.Module):
    ...
    def __init__(self, x_dim: int, y_dim: int, r_dim: int, z_dim: int, hidden_dim: int):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(x_dim + y_dim, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, r_dim)
        )

        self.aggregator = lambda r: r.mean(dim=1)

        self.latent_encoder = nn.Sequential(
            nn.Linear(r_dim, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, z_dim * 2)
        )
