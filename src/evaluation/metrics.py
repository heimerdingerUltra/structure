...
class MetricsCalculator:
    ...
    def compute_regression_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> RegressionMetrics:
        ...
        return RegressionMetrics(
            rmse=rmse, mae=mae, mape=mape, r2=r2, mse=mse, max_error=max_error, median_ae=median_ae
        )

    @staticmethod
    def compute_quantile_metrics(
        y_true: np.ndarray, y_pred: np.ndarray, quantiles: List[float] = [0.1, 0.25, 0.5, 0.75, 0.9]
    ) -> Dict:
    ...
    def compute_segment_metrics(
        y_true: np.ndarray, y_pred: np.ndarray, segments: np.ndarray
    ) -> Dict:
        ...
        for segment in np.unique(segments):
            ...
            if np.sum(mask) > 0:
                metrics = MetricsCalculator.compute_regression_metrics(y_true[mask], y_pred[mask])
...
class FinancialMetrics:
    ...
    @staticmethod
    def compute_iv_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> Dict:
...
class EnsembleMetrics:
    ...
    @staticmethod
    def compute_disagreement(predictions_list: List[np.ndarray], threshold: float = 2.0) -> Dict:
...
class MetricsSummary:
    ...
    def create_summary(
        ...
        summary = {**basic_metrics.to_dict(), **quantile_metrics, **directional}
