    ...
    def update(self):
        ...
        with torch.no_grad():
            ...
            for name in model_params.keys():
                shadow_params[name].data.mul_(decay).add_(model_params[name].data, alpha=1 - decay)
...
class SWA:
    ...
    def update(self):
        ...
        with torch.no_grad():
            ...
            for name in model_params.keys():
                swa_params[name].data.mul_(self.n_averaged - 1).add_(model_params[name].data).div_(
                    self.n_averaged
                )
    ...
    def state_dict(self):
        return {"swa_model": self.swa_model.state_dict(), "n_averaged": self.n_averaged}
        ...
        with torch.no_grad():
            ...
            for name in model_params.keys():
                avg_params[name].data.mul_(self.alpha).add_(
                    model_params[name].data, alpha=1 - self.alpha
                )
