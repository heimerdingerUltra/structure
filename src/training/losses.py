...
class WingLoss(nn.Module):
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        loss = torch.where(
            diff < self.omega, self.omega * torch.log(1.0 + diff / self.epsilon), diff - self.C
        )
...
class AdaptiveWingLoss(nn.Module):
    ...
    def __init__(
        self, omega: float = 14.0, theta: float = 0.5, epsilon: float = 1.0, alpha: float = 2.1
    ):
        super().__init__()
        self.omega = omega
        self.theta = theta
...
class CombinedLoss(nn.Module):
    ...
    def __init__(
        self, loss_types: list = ["huber", "mse"], weights: list = [0.7, 0.3], delta: float = 1.0
    ):
        super().__init__()
...
class AsymmetricLoss(nn.Module):
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        loss = torch.where(diff > 0, self.over_penalty * diff**2, self.under_penalty * diff**2)
