...
class TestDataPipeline(unittest.TestCase):
    ...
    def test_create_loaders(self):
        ...
        train_loader, val_loader, test_loader = pipeline.create_loaders(
            self.X, self.y, test_size=0.2, val_size=0.1, seed=42
        )
    ...
    def test_kfold_loaders(self):
        ...
        folds = list(pipeline.create_kfold_loaders(self.X, self.y, n_splits=3, seed=42))
...
class TestEnsemble(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.models = {
            "model1": ModelFactory.create_tabpfn(
                self.n_features, {"d_model": 64, "n_layers": 2, "n_heads": 4}
            ),
    ...
    def test_attention_ensemble(self):
        ensemble = AdvancedEnsembleSystem(self.models, device="cpu", ensemble_type="attention")
    ...
    def test_hierarchical_ensemble(self):
        ensemble = AdvancedEnsembleSystem(self.models, device="cpu", ensemble_type="hierarchical")
    ...
    def test_uncertainty_ensemble(self):
        ensemble = AdvancedEnsembleSystem(self.models, device="cpu", ensemble_type="uncertainty")
...
class TestMetrics(unittest.TestCase):
    ...
    def test_regression_metrics(self):
        metrics = MetricsCalculator.compute_regression_metrics(self.y_true, self.y_pred)
    ...
    def test_quantile_metrics(self):
        metrics = MetricsCalculator.compute_quantile_metrics(self.y_true, self.y_pred)

        self.assertIn("error_q50", metrics)
        self.assertIn("error_q90", metrics)

    def test_directional_accuracy(self):
        metrics = MetricsCalculator.compute_directional_accuracy(self.y_true, self.y_pred)
...
class TestConfiguration(unittest.TestCase):
    ...
    def test_model_config_creation(self):
        config = create_model_config(ModelType.TABPFN, n_features=20, d_model=256)
