...
from core.tensor_ops import fused_swiglu, rms_norm
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, layer_id: int, config: TransformerConfig):
        ...
        attn_config = AttentionConfig.from_dim(
            config.dim, config.n_heads, config.n_kv_heads, config.dropout, bias=False
        )
    ...
    def forward(
        self, x: Tensor, mask: Optional[Tensor] = None, cache: Optional[tuple] = None
...
class Transformer(nn.Module):
    ...
    def __init__(self, config: TransformerConfig):
        ...
        self.layers = nn.ModuleList([TransformerBlock(i, config) for i in range(config.depth)])

        self.norm = RMSNorm(config.dim, config.norm_eps)

    def forward(
        self, x: Tensor, mask: Optional[Tensor] = None, cache: Optional[list] = None
...
class VolatilityTransformer(nn.Module):
    ...
    def __init__(self, n_features: int, config: TransformerConfig, pooling: str = "attention"):
        super().__init__()
...
class ResidualBlock(nn.Module):
    ...
    def __init__(
        self, dim: int, hidden_dim: int, dropout: float = 0.0, activation: Callable = nn.SiLU
    ):
        super().__init__()
...
class ModernMLP(nn.Module):
    ...
    def __init__(
        ...
        self.stem = nn.Sequential(
            nn.Linear(n_features, hidden_dims[0]), RMSNorm(hidden_dims[0]), activation()
        )

        self.blocks = nn.ModuleList(
            [ResidualBlock(dim, dim * 4, dropout, activation) for dim in hidden_dims]
        )
...
class DenseNet(nn.Module):
    ...
    def __init__(
        ...
        self.stem = nn.Sequential(nn.Linear(n_features, init_dim), RMSNorm(init_dim), nn.SiLU())
