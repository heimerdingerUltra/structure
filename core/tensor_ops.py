...
def grouped_query_attention(
    query: Tensor, key: Tensor, value: Tensor, n_groups: int, mask: Optional[Tensor] = None
) -> Tensor:
...
def apply_rotary_embeddings(x: Tensor, cos: Tensor, sin: Tensor) -> Tensor:
...
def flash_attention_forward(
    query: Tensor, key: Tensor, value: Tensor, block_size: int = 128
) -> Tensor:
...
def compute_rope_embeddings(
    dim: int, seq_len: int, device: torch.device, base: float = 10000.0
) -> Tuple[Tensor, Tensor]:
...
def fused_layernorm_residual(
    x: Tensor, residual: Tensor, weight: Tensor, bias: Tensor, eps: float = 1e-6
) -> Tuple[Tensor, Tensor]:
...
def efficient_attention(
    query: Tensor, key: Tensor, value: Tensor, use_flash: bool = True, block_size: int = 128
) -> Tensor:
...
def tensor_parallel_linear(x: Tensor, weight: Tensor, n_devices: int = 1) -> Tensor:
