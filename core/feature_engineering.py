...
class OptionCharacteristics:
    ...
    def vega_proxy(self, vol: float = 0.2) -> np.ndarray:
        d1 = (self.log_moneyness + 0.5 * vol**2 * self.time_to_expiry) / (
            vol * np.sqrt(self.time_to_expiry) + 1e-10
        )
        return (
            self.spot * np.exp(-0.5 * d1**2) / np.sqrt(2.0 * np.pi) * np.sqrt(self.time_to_expiry)
        )
    ...
    def gamma_proxy(self, vol: float = 0.2) -> np.ndarray:
        ...
        return np.exp(-0.5 * d1**2) / (
            self.spot * vol * np.sqrt(2.0 * np.pi * self.time_to_expiry) + 1e-10
        )
