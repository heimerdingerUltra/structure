...
class DistributedStratifiedSampler(Sampler):
    ...
    def __init__(
        ...
        self.bin_indices = [
            np.where((targets >= bins[i]) & (targets < bins[i + 1]))[0] for i in range(num_bins)
        ]
...
class DataModule:
    ...
    def create_loaders(
        ...
        if self.use_stratified_sampling:
            train_sampler = DistributedStratifiedSampler(
                y_train, num_replicas=1, rank=0, shuffle=True, num_bins=10
            )
            shuffle_train = False
        else:
            train_sampler = None
    ...
    def kfold_split(
        self, X: np.ndarray, y: np.ndarray, n_folds: int = 5, seed: int = 42
