...
class FeatureEngineering:
    ...
    def _compute_hash(self, df: pd.DataFrame) -> str:
        data_hash = hashlib.blake2b(df.values.tobytes(), digest_size=16).hexdigest()
        return data_hash
    ...
    def _microstructure_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if {"BIDSIZE", "ASKSIZE"}.issubset(df.columns):
            ...
            features["weighted_mid"] = (bid * ask_size + ask * bid_size) / total_size

            features["effective_spread"] = (features["weighted_mid"] - mid) / mid

            features["microprice"] = (bid * ask_size + ask * bid_size) / (
    ...
    def _moneyness_features(self, df: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        strike_col = next(
            (c for c in ["STRIKE_PRC", "STRIKE_PRICE", "STRIKE"] if c in df.columns), None
        )
        ...
        for level in moneyness_levels:
            features[f"moneyness_above_{int(level*100)}"] = (moneyness > level).astype(np.float32)
    ...
    def _liquidity_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if {"ACVOL_1", "OPINT_1"}.issubset(df.columns):
            ...
            features["activity_ratio"] = volume / (volume + oi + 1e-10)
    ...
    def _interaction_features(self, features: pd.DataFrame) -> pd.DataFrame:
        if "moneyness" in features.columns and "tte_years" in features.columns:
            ...
            features["log_m_log_t"] = features["log_moneyness"] * features["log_tte"]
        ...
        if "spread_rel" in features.columns and "sqrt_tte" in features.columns:
            features["spread_sqrt_t"] = features["spread_rel"] * features["sqrt_tte"]
            features["spread_inv_t"] = features["spread_rel"] * features["inv_tte"]
        ...
        if "log_volume" in features.columns and "moneyness" in features.columns:
            ...
            features["volume_atm_distance"] = features["log_volume"] * features["atm_distance"]

        if "imbalance" in features.columns and "spread_rel" in features.columns:
            features["imbalance_spread"] = features["imbalance"] * features["spread_rel"]

        if {"log_liquidity", "atm_distance", "sqrt_tte"}.issubset(features.columns):
            features["liquidity_atm_time"] = (
                features["log_liquidity"] * features["atm_proximity"] * features["sqrt_tte"]
            )
    ...
    def _option_greeks_proxy(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        features["gamma_proxy"] = np.exp(-0.5 * (log_m / (sqrt_t + 1e-10)) ** 2) / (
    ...
    def fit_transform(
        self, df: pd.DataFrame, target_col: str = "IMP_VOLT", use_cache: bool = True
        ...
        if "OPTION_TYPE" in df.columns:
            features["is_call"] = (df["OPTION_TYPE"].str.upper() == "CALL").astype(np.float32)
        ...
        valid_mask = (y > 0) & (y < 500) & ~np.isnan(y) & ~np.isinf(y)
        ...
        self.metadata = FeatureMetadata(
            names=feature_names, statistics=feature_stats, version="1.0.0", n_samples=len(X_valid)
        )
